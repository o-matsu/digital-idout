...
export const actions = {
  ...
  async getRoleRegions(_, { regions, role }) {
    const regionDocs = await this.$fire.firestore
      .collection('regions')
  ...
  async fetchMetasByRegion({ rootGetters }, regionId) {
    ...
    const metas = []
    const metaDocs = await this.$fire.firestore
      .collection('metas')
  ...
  async insertRegion(_, { points }) {
    ...
    try {
      const regionRef = await this.$fire.firestore
        .collection('regions')
        .add(data)
      return regionRef.id
    } catch (e) {
  ...
  async deleteMeta({ dispatch }, { meta }) {
    try {
      ...
      const hasRole = await this.$fire.firestore
        .collection('metas')
        .where('regionId', '==', regionId)
        .where('targetRole', '==', deletedRole)
        .get()
      if (hasRole.empty) {
        const regionRef = this.$fire.firestore
          .collection('regions')
          .doc(regionId)
        const regionDoc = await regionRef.get()
        const docData = regionDoc.data().hasRoles
